...
async def start(event):
  await event.reply("**🌀TagXaosbot**\n ile Grubunuzdakı Nerdeyse Tüm Üyelere Etiket Ata bilirim \nKomutlar için =======> /help yazın**",
		      ...
		      [Button.url('Sahibi👨🏻‍💻', 'https://t.me/@mr_kabus_13')],
                    ),
                    link_preview=False
                   )
@client.on(events.NewMessage(pattern="^/help$"))
async def help(event):
  helptext = "**🌀 TagXaosbot Kamandalar**\n\n**/tag <sebeb> - 5-li Tag Atar**\n\n**/etag <sebeb> - Emoji ile taglar**\n\n**/tektag sebeb - Userleri tek tek tağ atar**\n\n**/admins sebeb - Adminleri Tek Tek Tağ Eder**\n\n**/start - botu başladın**"
  await event.reply(helptext,
                    buttons=(
                      [Button.url('meni Gruba Ekle➕', 'https://t.me/tagxaosbot?startgroup=a')],
                      [Button.url('Fban kanal👨‍💻', 'https://t.me/x_fedd')],
                      [Button.url('Resmi Kanal🔖', 'https://t.me/xxaosresmi')],
		      [Button.url('Sahib🧑‍🔧', 'https://t.me/mr_kabus_13')],
                    ),
                    link_preview=False
                   )
...
async def help(event):
  helptext = "**çox funksiyalı tağ botu axdaran @TagXaosbot Size Göre:\n\n📌 5-li tağ\n📌 Emoji tağ\n📌 Tekli tağ\n📌 Yalnız adminleri tag edər\n📌\n\n bele çox funksiyalı @TagXaosbot 'u grubunuza admin olarag ekleyib rahatlığa işlədə bilərsiz , tag ata bilirsiz **"
  await event.reply(helptext,
                    buttons=(
...
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Bu kamandalar gruplar ve kanallar üçündür❗**")
  ...
  if event.pattern_match.group(1):
    ...
    return await event.respond("tag üçün sebeb yaz❗️")
  else:
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in anlik_calisan:
        await event.respond("** Etiket işlemi uğurla dayandırıldı❌**")
        return
      if usrnum == 5:
  ...
  if mode == "text_on_reply":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      usrtxt += f"[{random.choice(emoji)}](tg://user?id={usr.id}) "
      if event.chat_id not in anlik_calisan:
        await event.respond("Dayandırıldı hec bir etiket baş vermir\n\n**Burda sizin reklamınız ola bilir @LuciBots**❌")
        return
      if usrnum == 5:
